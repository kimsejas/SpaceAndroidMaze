...
import edu.wm.cs.cs301.amazebykimberlysejas.generation.BSPBranch;
import edu.wm.cs.cs301.amazebykimberlysejas.generation.BSPLeaf;
import edu.wm.cs.cs301.amazebykimberlysejas.generation.BSPNode;
import edu.wm.cs.cs301.amazebykimberlysejas.generation.Floorplan;
import edu.wm.cs.cs301.amazebykimberlysejas.generation.Wall;
...
//        gc = (Graphics2D) g ;
        ...
        drawBackground(p, percentToExit);
...
//		panel = p;
		p.setColor((ColorTheme.getColor(ColorTheme.MazeColors.FIRSTPERSON_DEFAULT).toArgb()));
		...
		//drawing walls
		drawAllVisibleSectors(p, bspRoot);
	 ...
	 */
	private void drawBackground(MazePanel p, float percentToExit) {
		p.addBackground(percentToExit);
...
//		panel.fillRect(0, viewHeight/2, viewWidth, viewHeight/2);
	 ...
	 */
	private void drawAllVisibleSectors(MazePanel p, BSPNode node) {
		traverseNodeCounter++; // debug

		// Anchor, stop recursion at leaf nodes
		if (node.isIsleaf()) {
			drawAllWallsOfASector(p, (BSPLeaf) node);
			return;
		}
		...
		if ((dot >= 0) && (boundingBoxIsVisible(right))) {
			drawAllVisibleSectors(p, right);
		}
		// consider left node
		BSPNode left = n.getLeftBranch();
		if (boundingBoxIsVisible(left))
			drawAllVisibleSectors(p, left);
	 ...
	 */
	private void drawAllWallsOfASector(MazePanel p, BSPLeaf node) {
	 ...
	 */
	private void drawWall(MazePanel p, Wall wall) {
		...
		p.setColor(ColorTheme.getColor(wall.getColor()).toArgb());
//		Log.v("color", "🦜🦜🎨color is " + ColorTheme.getColor(wall.getColor()));
//		p.setColor(Color.RED);
		boolean drawn = drawPolygons(p, x1, x2, y11, y12, y21, y22);
	 ...
	 */
	private boolean drawPolygons(MazePanel p, int x1, int x2, int y11, int y12, int y21, int y22) {
		// debugging
